...
def test(run) -> QualityCheckResult:
    ...
    return _run(QualityCheckSpec(
        ...
        area="backend",
        operation="build",
        # Both articlegen suites, in the order the README runs them. cmd /c
        # chains them so one argv covers both; the second only runs if the
        # first passes, which matches "run both before shipping".
        argv=["cmd", "/c",
